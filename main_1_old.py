...
def get_documents():
    if request.method == "POST":
        ...
        if request.args.get('query') != None:
            ...
            print("Keywords & Weights: {0}".format(session['query_weights']))
            print('\n')            
            out['query_weights'] = session['query_weights']
            out['query'] = session['query']

        if request.args.get('spec') != None:
            session['spec'] = request.args.get('spec')
            out['spec'] =  session['spec']

        if request.args.get('level') != None:
            session['level'] = request.args.get('level')
            out['level'] = session['level']
        return(out)
        ...
        if query != None:
            try:
                ...
                try:
                    ...
                    print(e)
                conn.commit()
                return ({'Selected Concentration' : spec,'Selected Level': level , 'query' : query, 'Documents' : output})
            except Exception as E:
                return(E)
    else:
        return("Method Error")
